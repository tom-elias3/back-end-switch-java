...
**External routing file:** Set env var `ROUTING_PROPERTIES_PATH` (Spring property `routing.properties.path`) to load patterns from a file outside the JAR — useful for Docker volume mounts and Kubernetes ConfigMaps. If unset, falls back to the classpath file. `POST /reload` re-reads from the same path. When set, `RoutingFileWatcher` watches the file for changes and calls `init()` automatically; disable with `ROUTING_FILE_WATCHER_ENABLED=false` (Spring property `routing.file-watcher.enabled`, default `true`).
...
- `service` — `DecisionService` — orchestration: pattern loading/reloading, `handleRequest`, `matchPattern`, `evaluateLogic`. `DecisionHelper` (plain static utility class, no Spring wiring) — pure functions: `extractClaims` (catches all exceptions, returns empty map on any malformed token), `extractRequestParams`, `matchUrl`, `flattenPayload`, `probabilityDecision`, `proxyRequest`. `RoutingFileWatcher` — watches the external routing file for changes and calls `decisionService.init()` automatically; only active when `ROUTING_PROPERTIES_PATH` is set and `routing.file-watcher.enabled` is `true`; daemon thread, stopped on `@PreDestroy`
