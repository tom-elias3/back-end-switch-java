...
- `service` — `DecisionService` — orchestration: pattern loading/reloading, `handleRequest`, `matchPattern`, `evaluateLogic`. `DecisionHelper` (plain static utility class, no Spring wiring) — pure functions extracted from `DecisionService`: `extractClaims`, `extractRequestParams`, `matchUrl`, `flattenPayload`, `probabilityDecision`, `proxyRequest`
